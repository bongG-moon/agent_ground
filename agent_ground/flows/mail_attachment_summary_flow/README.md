...
- EWS가 없는 테스트 환경에서는 `mail_attachment_summary_dummy_flow.json`을 사용해 네트워크 호출 없이 메일 본문, TXT 첨부와 JPG Vision 예제를 생성할 수 있습니다.
...
2. `01T 테스트 EWS 메일·첨부 데이터`에서 더미 메일 수, 첨부 포함과 `JPG Vision 예제 포함`을 선택합니다.
3. DRM 노드는 `자동(로컬 우선)`을 유지합니다. TXT 첨부는 로컬에서 처리되므로 DRM API를 호출하지 않습니다.
4. `03A`에 사내 Vision 모델을 연결합니다. 첫 번째 메일의 `vision_demo_status_board.jpg`가 자동으로 이 경로를 사용합니다.
5. 두 요약 Language Model에 사내 승인 모델을 선택하고 Chat Output까지 실행합니다.

기본 설정은 메일 2통, TXT 첨부 2개와 첫 번째 메일의 JPG 예제 1개를 만들어 총 5개 Loop 항목이 됩니다. JPG에는 `VISION TEST`, `TEMP SENSOR 2`, `STATUS: CHECK` 표지판이 들어 있습니다. 03A가 정상 연결되면 `processing_path=vision_model`, `vision_status=text_extracted`가 되고, 연결하지 않으면 Loop를 중단하지 않고 `vision_model_missing` 안내 TXT가 생성됩니다. 생성 행은 운영 EWS와 동일하게 `mail_index`, `mail_subject`, `sender`, `received_time`, `file_path`, `source_kind`, `drm_status` 등을 포함합니다.
