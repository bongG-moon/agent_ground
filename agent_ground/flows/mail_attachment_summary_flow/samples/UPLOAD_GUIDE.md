...
3. `03B EWS 문서·이미지·미지원 첨부 처리`에서 처리 모드를 선택합니다.
4. 자동 fallback 또는 항상 DRM을 사용하면 DRM API 주소, Bearer 토큰과 사번을 입력합니다.
5. `03A JPG·PNG 이미지 해석 모델 (vLLM)`에 사내 승인 멀티모달 모델을 선택합니다.
...
- JPG/JPEG/PNG는 연결된 Vision 모델로 해석하며, 그 밖의 이미지는 DRM API가 OCR 평문을 반환하는 범위에서 읽습니다.
