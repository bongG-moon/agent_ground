...
def test_flow_contains_expected_ews_drm_graph() -> None:
    ...
    assert file_to_formatter["data"]["sourceHandle"]["output_types"] == ["Table"]
    assert file_to_formatter["data"]["targetHandle"]["inputTypes"] == ["DataFrame", "Table"]
...
def test_dummy_flow_replaces_only_ews_source_and_keeps_typed_pipeline() -> None:
    ...
    assert any(
        ...
        and edge["data"]["sourceHandle"]["output_types"] == ["DataFrame", "Table"]
        for edge in edges
    )
...
def test_vision_coroutine_factory_is_visible_to_langflow_1_9_2_loader() -> None:
    module = ast.parse(DRM_SOURCE.read_text(encoding="utf-8"))
    matching_nodes = [
        node
        for node in module.body
        if getattr(node, "name", "") == "process_file_record_with_vision"
    ]

    assert len(matching_nodes) == 1
    assert isinstance(matching_nodes[0], ast.FunctionDef)
    assert not isinstance(matching_nodes[0], ast.AsyncFunctionDef)
...
def test_dummy_rows_and_formatter_run_without_ews_or_drm(
    ...
    drm_module = load_module(DRM_SOURCE, "mail_flow_dummy_vision_drm_test")
    rows = dummy_module.build_dummy_mail_rows(
        mail_count=2,
        include_attachments=True,
        include_vision_attachment=True,
    ...
    assert rows[2]["file_name"] == "vision_demo_status_board.jpg"
    assert rows[2]["content_type"] == "image/jpeg"
    assert Path(rows[2]["file_path"]).read_bytes().startswith(b"\xff\xd8\xff")
    ...
    class VisionModel:
        async def ainvoke(self, _messages):
            return SimpleNamespace(content="TEMP SENSOR 2 상태는 CHECK로 표시되어 있습니다.")

    vision_result = asyncio.run(
        drm_module.process_file_record_with_vision(
            rows[2],
            VisionModel(),
            output_root=tmp_path / "dummy-vision-output",
        )
    )
    assert vision_result["processing_path"] == "vision_model"
    assert vision_result["vision_status"] == "text_extracted"
    assert "TEMP SENSOR 2" in Path(vision_result["file_path"]).read_text(encoding="utf-8")
    ...
    dummy_component.include_vision_attachment = True
