...
_FALLBACK_JPEG_BASE64 = (
    "/9j/4AAQSkZJRgABAQAAAQABAAD/2wBDABALDA4MChAODQ4SERATGCgaGBYWGDEjJR0oOjM9PDkzODdASFxO"
    "QERXRTc4UG1RV19iZ2hnPk1xeXBkeFxlZ2P/2wBDARESEhgVGC8aGi9jQjhCY2NjY2NjY2NjY2NjY2NjY2"
    "NjY2NjY2NjY2NjY2NjY2NjY2NjY2NjY2NjY2NjY2NjY2NjY2P/wAARCAAIAAgDASIAAhEBAxEB/8QAFQAB"
    "AQAAAAAAAAAAAAAAAAAAAAf/xAAUEAEAAAAAAAAAAAAAAAAAAAAA/8QAFAEBAAAAAAAAAAAAAAAAAAAAAP/E"
    "ABQRAQAAAAAAAAAAAAAAAAAAAAD/2gAMAwEAAhEDEQA/AKAAD//Z"
)
...
def _build_vision_demo_jpeg() -> bytes:
    """Pillow가 있으면 텍스트 표지판을, 없으면 유효한 최소 JPEG를 반환합니다."""
    ...
    try:
        ...
        draw.rectangle((18, 18, 461, 251), outline="#1f4e79", width=6)
        draw.text((42, 45), "VISION TEST", fill="#1f4e79")
        draw.text((42, 105), "TEMP SENSOR 2", fill="black")
        draw.text((42, 165), "STATUS: CHECK", fill="#c00000")
        draw.ellipse((365, 135, 425, 195), fill="#c00000")
        buffer = io.BytesIO()
        image.save(buffer, format="JPEG", quality=80, optimize=True)
        return buffer.getvalue()
    except (ImportError, OSError):
        return base64.b64decode(_FALLBACK_JPEG_BASE64)
...
def build_dummy_mail_rows(
    *,
    ...
    include_vision_attachment: bool = True,
    ...
    for index in range(1, count + 1):
        ...
        if include_attachments:
            ...
            if include_vision_attachment and index == 1:
                vision_name = "vision_demo_status_board.jpg"
                vision_path = mail_dir / f"002_{vision_name}"
                vision_path.write_bytes(_build_vision_demo_jpeg())
                rows.append(
                    {
                        **common,
...
class DummyEwsMailItems(Component):
    ...
    inputs = [
        ...
        BoolInput(
            name="include_vision_attachment",
            display_name="JPG Vision 예제 포함",
            value=True,
            info=(
                "첫 번째 더미 메일에 실제 JPEG 첨부를 추가합니다. 03A Vision 모델을 설정하면 "
                "TEMP SENSOR 2와 STATUS: CHECK 문구를 해석하는 경로를 확인할 수 있습니다."
            ),
        ),
        StrInput(
            name="scenario_label",
    ...
    def build_mail_items(self) -> DataFrame:
        rows = build_dummy_mail_rows(
            ...
            include_vision_attachment=bool(getattr(self, "include_vision_attachment", True)),
        )
