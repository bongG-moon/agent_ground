...
사내 규정·매뉴얼·FAQ를 질문할 때, 사용자가 볼 수 있는 문서만 검색하고 답변 문장과 출처를 함께 반환하는 Langflow `1.9.2` 실행 예시입니다. 6개 기능 단위 Component와 3개 Flow 내부 노드는 모두 파일 하나로 등록되는 Standalone 방식입니다.
...
이 항목들은 데모 계약이 확인된 뒤 각각 별도 ingestion flow, identity adapter, vector-store adapter, DLP service, LLM adapter로 교체해야 합니다. 기본 Milvus 검색으로 바꾸려면 ACL metadata filter와 stable-ID replacement 계약을 Langflow `1.9.2`에서 별도 검증해야 하며, 확인 전에는 이 예시의 안전한 retriever를 그대로 대체할 수 없습니다.
