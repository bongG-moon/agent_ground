...
## 3. Langflow 1.9.2에서 MCP 서버 먼저 등록하기
...
Langflow 1.9.2에서는 Settings의 MCP Servers에서 서버를 등록하고 MCP sidebar에서 Tool을 가져옵니다. 현재 사용자 DB에 서버가 없거나 권한이 없으면 Flow JSON에 같은 서버 이름이 남아 있어도 Tool을 불러올 수 없습니다.
...
- Langflow 1.9.2의 MCP Tools는 그래프의 Langflow 세션 ID와 서버 이름을 이용해 지속형 MCP 세션 Context를 구분할 수 있습니다.
