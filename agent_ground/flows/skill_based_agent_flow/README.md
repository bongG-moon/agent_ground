...
현재 버전은 `0.2.0`, 대상 환경은 Langflow `1.9.2` / LFX `0.4.2`, 공개 상태는 `user_testing`입니다. 실제 Agent 실행에는 조직에서 승인한 Tool Calling 지원 모델과 API Key가 필요합니다.
...
1. Langflow `1.9.2` Builder에서 `00_SKILL_BASED_AGENT_ALL_FLOWS.json`을 가져옵니다.
