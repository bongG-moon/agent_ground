...
```text
route 값이 data_retrieval, document_rag, time_answer, final_answer일 때 각각 어떤 흐름으로 가야 해?
```

기대 답변:

`data_retrieval`은 API/DB/CSV 같은 정형 데이터 조회 branch로 보내고, `document_rag`는 Milvus 또는 Vector Store 기반 문서 검색 branch로 보냅니다. `time_answer`는 `Current Time Context`를 붙여 현재 시간 기준 답변을 만들고, `final_answer`는 추가 조회 없이 final prompt 또는 질문 보정 답변 branch로 보냅니다. 처음 실습에서는 네 output을 각각 Chat Output에 연결해 `active=true`와 `skipped=true` payload를 먼저 확인합니다.

질문:

```text
오늘 기준 이번 주 실습 마감 전에 무엇을 먼저 해야 해?
```

기대 답변:

이 질문은 `time_answer` route로 보내고 `Current Time Context`의 `Time Message`를 Prompt Template의 `{current_time}`에 연결합니다. 답변에는 서버 기준 timezone과 현재 날짜/시간을 함께 고려하고, 문서나 데이터 조회가 필요한 내용은 추측하지 말고 추가 정보를 요청합니다.
