...
# time_answer는 "오늘/현재/마감"처럼 현재 시간 context가 필요한 질문을 별도 branch로 보내기 위한 예시입니다.
...
class RouteGate(Component):
    ...
    # group_outputs=True를 쓰면 네 output port가 동시에 노출됩니다.
    ...
    outputs = [
        ...
        Output(
            name="time_answer",
            display_name="Time Answer",
            method="to_time_answer",
            types=["Data"],
            group_outputs=True,
        ),
        Output(
    ...
    def to_time_answer(self) -> Data:
        # 시간/날짜 branch: Current Time Context와 Prompt Template을 붙여 상대 날짜 질문에 답합니다.
        return self._route_payload("time_answer")
