...
> 현재 기준: Langflow `1.9.2`, langflow-base `0.9.2`, LFX `0.4.2`, Python `3.12`

목표는 단순히 "Python class 하나를 Langflow 화면에 띄우는 법"을 넘어서, 다음 역량을 갖추는 것이다.

- Langflow custom node의 구조와 실행 흐름을 설명할 수 있다.
- 1.9.2 화면의 `JSON`, `Message`, `Table`과 Python 호환 클래스 `Data`, `Message`, `DataFrame`을 구분해 출력 타입을 선택할 수 있다.
...
- AI agent flow를 현재 대화 상태, 업무 규칙/데이터 목록, LLM 지시문, LLM 호출, 결과 해석, 분기, 조회/검색, 후처리, 최종 답변, 이전 대화 기억 단계로 나누어 설계할 수 있다.
- 노드가 실패해도 다음 node가 읽을 수 있는 결과를 반환하고, Langflow UI에서 디버깅 가능한 상태를 남길 수 있다.
- Langflow canvas에 붙여도 깨지지 않는 standalone component를 작성할 수 있다.

기준 문서는 Langflow 공식 문서의 Custom Components, Components overview, Tool mode 설명이다. 이 프로젝트의 모든 예제는 1.9.2/LFX 0.4.2 계약으로 검증하며, 이후 버전의 화면이나 import path를 미리 가정하지 않는다.

Langflow 1.9에서는 화면의 `Data`와 `DataFrame`이 주로 `JSON`과 `Table`로 표시된다. Standalone Python 코드에서는 호환을 위해 `Data`와 `DataFrame` 클래스를 계속 사용할 수 있다. 자세한 이관 규칙은 [`LANGFLOW_1_9_2_MIGRATION_GUIDE.md`](LANGFLOW_1_9_2_MIGRATION_GUIDE.md)를 참고한다.

## 먼저 풀어 읽는 용어

| 용어 | 쉽게 말하면 |
| --- | --- |
| payload | 다음 node로 넘기는 결과 묶음. 성공 여부, 오류, 미리보기 데이터처럼 뒤 node가 읽을 값을 담는다. |
| prompt | LLM에게 보내는 지시문. 무엇을 참고하고 어떤 형식으로 답할지 적는다. |
| schema | 결과의 모양 규칙. 예: `route`, `answer`, `errors` 같은 key와 타입을 정한다. |
| parser | 결과에서 필요한 값을 꺼내 사람이 읽기 좋게 바꾸는 node. |
| normalizer | 이름, 기본값, 형식을 사내 기준에 맞게 정리하는 node. |
| retriever | DB, API, 파일, 문서 저장소에서 필요한 근거를 찾아오는 조회/검색 담당 node. |
| embedding / vector | 문장의 의미를 숫자로 바꾼 값. RAG에서 비슷한 문서를 찾을 때 쓴다. |
| chunk | 긴 문서를 검색하기 좋게 작게 나눈 조각. |
| branch | 조건에 따라 갈라지는 흐름. 데이터 질문은 데이터 조회로, 문서 질문은 RAG 검색으로 보낸다. |

## 1. 개발자가 가져야 할 기본 관점

Langflow에서 하나의 node는 "화면에 보이는 Python class"이다. 하지만 AI agent flow에서 좋은 node는 단순 class가 아니라 작은 약속 단위이다. 여기서 약속이란 "무엇을 받고, 무엇을 내보내고, 실패하면 어떤 모양으로 알려줄지"를 미리 정한다는 뜻이다.
...
| 어떤 output을 내보내는가? | 화면의 `JSON`, `Message`, `Table` 중 하나로 명확하다. Python 구현 클래스도 함께 기록한다. |
| 실패하면 어떻게 되는가? | `success=False`, `errors=[]` 등 다음 node가 읽을 수 있는 형태로 반환한다. |
| 다음 node는 무엇을 기대하는가? | 결과 묶음의 key 이름이 안정적이고 문서화되어 있다. |
| Langflow UI에서 확인할 수 있는가? | `self.status`, 점검 정보, 경고 메시지가 적절하다. |

반대로 좋지 않은 node는 다음 특징을 가진다.

- LLM 지시문 생성, LLM 호출, JSON 읽기, DB 조회, pandas 계산, 최종 답변을 한 파일에 모두 넣는다.
- output method 반환 타입이 `-> Any`라서 포트 타입이 흐려진다.
- `dict`나 `str`를 그대로 반환해서 다음 node 연결이 불안정하다.
- API key, password, token을 일반 텍스트 input이나 점검용 결과 묶음에 노출한다.
- domain-specific rule을 Python 코드에 하드코딩한다.
- 오래 걸리는 외부 호출에 timeout이 없다.
- 큰 row list 전체를 대화 상태나 LLM 지시문에 계속 싣는다.

이 repo의 `langflow_main/` 원칙도 같다. component 파일은 파일 하나만 붙여넣어도 실행되도록 유지하고, 제조 도메인별 규칙은 가능한 Python 코드가 아니라 업무 항목, main flow filter, table catalog 같은 관리 데이터에 둔다.
...
| `display_name` | Input, Output | 화면에 보이는 이름 | 사용자에게 보일 짧은 이름 |
| `info` | Input | 입력 도움말 | 어떤 값을 넣는지 한 문장으로 설명 |
| `value` | Input | 기본값 | 처음 실행해도 실패하지 않는 안전한 값 |
| `required` | Input | 필수 여부 | 없으면 실행할 수 없는 값만 `True` |
| `advanced` | Input | 고급 영역 표시 | 점검 정보, 제한 시간, 처리 개수, 기준값에 사용 |
| `input_types` | `DataInput`, `DataFrameInput` 등 | 받을 수 있는 연결 타입 제한 | `["Data", "JSON"]`처럼 명시 |
| `options` | `DropdownInput` | 선택지 목록 | 실행 방식, 제공사, route처럼 고정된 값에 사용 |
| `real_time_refresh` | Input | 이 입력값이 바뀌면 입력 화면을 다시 정리할 수 있음을 표시 | 선택값에 따라 다른 입력의 표시 여부, 필수 여부, 선택지를 바꿀 때만 사용 |
| `dynamic`, `show` | Input | 조건부 입력칸 표시 | 인증 방식에 따라 token 입력칸 표시 같은 경우 |
| `tool_mode` | 일부 Input | Agent가 tool 호출 시 채울 값으로 노출 | 질문, route, limit처럼 Agent가 판단해 넣을 값에만 사용 |
| `types` | `Output` | output 연결 타입 힌트 | 반환 객체와 맞게 `["Data"]`, `["Message"]`, `["DataFrame"]` |
| `method` | `Output` | 실행할 함수 이름 | 실제 함수 이름과 정확히 일치해야 함 |
...
Component class에는 보통 다음 class-level 속성을 둔다. 이 값들은 대부분 Langflow 화면에서 node를 어떻게 보여줄지 정하는 정보다.
...
Input은 단순히 화면 입력칸이 아니라 "이 node가 무엇을 받을지" 정하는 절반의 약속이다.

좋은 input은 다음 조건을 만족한다.

- `name`이 코드와 결과 묶음에서 쓰기 좋은 snake_case이다.
- `display_name`이 화면에서 읽기 쉽다.
- `info`가 "무엇을 넣어야 하는지"를 설명한다.
- 필수값과 운영 옵션을 구분한다.
- API key, password, token은 `SecretStrInput`이나 Langflow variable을 사용한다.
- 복잡한 JSON은 `MultilineInput` 또는 `DataInput`으로 받고, 함수 내부에서 표준 dict/text로 정리한다.
- 큰 데이터는 가능하면 `DataFrameInput`, `FileInput`, `data_ref`를 사용하고 LLM 지시문에 원문 전체를 넣지 않는다.

### 5.1 자주 쓰는 input class

| Input class | 용도 | 예시 | 주의점 |
| --- | --- | --- | --- |
| `MessageTextInput` / `StrInput` | 짧은 텍스트 | 질문, session id, model name | 긴 JSON/LLM 지시문에는 부적합 |
| `MultilineInput` | 긴 텍스트 | LLM 지시문, JSON, 정책 문서 | JSON을 dict로 읽는 helper를 함께 둔다 |
| `DataInput` | 다른 node의 `Data`/`JSON` 결과 묶음 | 상태, 의도, 업무 데이터 | `input_types=["Data", "JSON"]` 권장 |
| `DataFrameInput` | 표 형태 데이터 | pandas table, CSV load 결과 | 다음 node가 Table을 기대할 때 사용 |
| `FileInput` | 파일 업로드/경로 | CSV, JSON, TXT, XLSX | 파일 객체/문자열/list 모두 방어적으로 처리 |
| `SecretStrInput` | 비밀값 | API key, DB password, token | status/점검 결과에 절대 출력하지 않음 |
| `BoolInput` | true/false 선택 | include_preview, dry_run, debug_mode | 오래된 환경에서는 문자열도 처리 |
| `IntInput` | 정수 | limit, timeout, max_steps | 최소/최대 범위 보정 필요 |
| `FloatInput` | 실수 | temperature, threshold | 최소/최대 범위 보정 필요 |
| `DropdownInput` | 선택지 | provider, parse_mode, auth_type | default 값을 반드시 지정 |
| `CodeInput` | 코드 입력 | pandas code, 변환 script | 실행 node는 반드시 제한 실행과 허용 목록 필요 |
| `HandleInput` / `ModelInput` | 실행 객체 연결 | LLM, embedding, tool | 객체를 `Data` 결과 묶음에 넣지 않음 |
| `PromptInput` | LLM 지시문 양식 | template 기반 지시문 | Agent tool mode와 함께 쓸 수 있음 |
...
| `display_name` | UI 이름 | 사람이 읽는 짧은 이름 |
| `info` | 도움말 | 운영자가 실수하기 쉬운 값에 작성 |
| `value` | 기본값 | safe default만 넣기 |
| `required` | 필수 입력 여부 | 없으면 실행할 수 없는 값에만 true |
| `advanced` | 고급 영역으로 숨김 | 제한 시간, 점검 정보, 처리 개수, 상태 필터 등 |
| `input_types` | 연결 가능한 타입 | `DataInput`에서는 `["Data", "JSON"]` 자주 사용 |
| `is_list` | list 입력 여부 | 여러 documents/tools를 받을 때 |
| `tool_mode` | Agent가 tool 호출 시 채울 값으로 노출 | 질문, route, limit처럼 Agent가 직접 채워야 하는 값에만 |
| `dynamic` | 처음에는 숨겨두거나 조건부로 보여줄 입력인지 표시 | 특정 옵션을 골랐을 때만 필요한 입력 |
| `real_time_refresh` | 이 입력값이 바뀌면 화면의 입력 구성을 다시 계산해야 함을 표시 | 선택값에 따라 다른 입력을 보이거나 숨길 때 |
| `options` | dropdown 선택지 | `DropdownInput`에서 사용 |

`real_time_refresh=True`는 "이 값을 바꾸면 component 실행 결과를 다시 만들라"는 뜻이 아니다. 사용자가 dropdown이나 toggle 값을 바꿨을 때 입력 화면 자체를 다시 정리하라는 표시다. 예를 들어 인증 방식을 `none`에서 `bearer`로 바꾸면 API key 입력칸을 보여주고 필수값으로 바꿀 수 있다. 외부 API 호출, 파일 읽기, LLM 호출처럼 실제 비용이 드는 작업은 output method에서 실행한다.
...
파일을 받는 node는 실제 agent 개발에서 자주 필요하다. CSV, JSON, JSONL, TXT, Markdown, Excel을 업로드해서 업무 규칙, 테이블 목록, 테스트 데이터셋, LLM 지시문 참고 정보로 넘길 수 있다.
...
- `dry_run`: DB 저장 전 결과 묶음만 확인
- `include_preview`: preview rows 포함 여부
- `include_debug`: 점검용 결과 포함 여부
- `strict_mode`: 결과 모양 오류 시 실패 처리할지 여부
...
- 다음 node가 Langflow Table/DataFrame component라면 `DataFrame`.
- custom node 사이에서 결과 모양, 오류, 문서/데이터 정보도 함께 보내야 한다면 `Data`.
- LLM 지시문에 표를 줄 때는 DataFrame 전체보다 미리보기와 요약을 사용한다.
...
- 허용된 실행 값, 제한 시간, 금지 import, row 제한을 둔다.
- 실행 결과와 오류를 결과 묶음에 분리한다.
- LLM이 만든 code는 정리 node가 금지 패턴을 검사한 뒤 실행 node로 넘긴다.

AI agent flow에서는 `LLM -> JSON 읽기 -> 실행 계획 정리 -> 제한된 실행` 구조로 나누는 것이 좋다.

### 6.8 `HandleInput`과 `ModelInput`

`HandleInput`은 model, embedding, retriever, tool 같은 Python 객체를 연결할 때 사용한다. 이 값은 JSON으로 저장되는 결과 묶음이 아니라 실행 중에만 쓰는 객체이다.

주의:

- `Data(data=...)` 안에 handle 객체를 넣지 않는다.
- 파일 하나로 배포하는 것이 중요하면 외부 객체 연결보다 명시적인 설정 input이 더 재현 가능하다.
- Agent tool 연결에서는 Langflow의 Tool output과 Agent Tools input 구조를 우선 고려한다.

## 7. Output 설계

Output은 "이 node가 무엇을 내보낼지" 정하는 나머지 절반의 약속이다.
...
| --- | --- | --- |
| `Data` | JSON처럼 key/value로 된 결과 묶음 | `DataInput`, JSON/Data 연결점 |
...
분기 node에서는 선택되지 않은 갈래도 명확히 반환한다.
...
뒤쪽에서 갈래를 합치는 node는 `active=True`이고 `skipped=False`인 갈래만 선택한다.

route 값별 flow 예시는 다음처럼 읽는다.

| route 값 | 선택되는 output | 다음 flow | 예시 질문 |
| --- | --- | --- | --- |
| `data_retrieval` | `Route Gate.data_retrieval` | API/DB/CSV 조회, DataFrame 분석, Parser/LLM 지시문 답변 | `defect_rate가 가장 높은 lot을 찾아줘` |
| `document_rag` | `Route Gate.document_rag` | Milvus/Vector Store 검색, 문서 근거 지시문, LLM 답변 | `RAG 운영 기준을 알려줘` |
| `time_answer` | `Route Gate.time_answer` | Current Time Context, Prompt Template, Chat Output | `오늘 기준 이번 주 실습 마감 전에 무엇을 먼저 해야 해?` |
| `final_answer` | `Route Gate.final_answer` | 조회 없이 최종 지시문 또는 질문 보정 답변 | `실습 순서를 간단히 알려줘` |

분기 flow를 처음 만들 때는 여러 output을 곧바로 API/DB/LLM에 연결하지 말고, 먼저 각각 `Chat Output`에 연결해서 선택된 갈래와 건너뛴 갈래의 결과 묶음을 눈으로 확인한다. 그 다음 실제 처리 노드로 교체한다.

각 갈래의 첫 노드는 선택되지 않은 결과 묶음을 받으면 외부 호출을 하지 않아야 한다.
...
# 선택된 갈래에서만 API, DB, Milvus, LLM 호출을 실행한다.
```

### 7.3 Smart Router와 현재 시간 갈래

Smart Router가 보이는 Langflow 환경에서는 사용자 질문을 `data_retrieval`, `document_rag`, `time_answer`, `final_answer` 같은 route 후보 중 하나로 분류하는 데 쓸 수 있다. Smart Router가 보이지 않는 버전이면 `Structured Output`으로 같은 JSON을 만들고, 그 결과를 `Smart Route Payload Builder`에 연결하면 된다.

권장 연결:

```text
Chat Input.Message
  -> Smart Router 또는 Structured Output
  -> Smart Route Payload Builder.router_decision
  -> Route Gate.intent_payload
  -> Route Gate.data_retrieval / document_rag / time_answer / final_answer
```

Structured Output 또는 Prompt Template에서 사용할 route 분류 지시문:
...
`time_answer` branch는 `Current Time Context`를 붙인다.

```text
Route Gate.time_answer
  -> Current Time Context.Time Message
  -> Prompt Template.{current_time}
...
## 8. Payload: 다음 node가 읽을 결과 묶음

Payload는 한 node가 만든 결과를 다음 node로 넘기기 위해 포장한 데이터다. Langflow custom component에서는 보통 `Data(data={...})` 안의 dict가 payload 역할을 한다.

초보자는 payload를 "다음 node가 읽을 영수증"처럼 이해하면 쉽다. 예를 들어 파일을 읽은 node가 `row_count`, `columns`, `preview_rows`, `errors`를 항상 같은 이름으로 넘기면, 뒤의 Prompt Builder나 Router는 매번 파일 형식마다 다른 코드를 작성하지 않아도 된다.

Custom node flow에서 가장 중요한 것은 결과 묶음의 key 이름을 안정적으로 유지하는 것이다.
...
- 사용자가 볼 답변과 개발자가 볼 점검 정보를 분리한다.
- 큰 원본 데이터는 저장소 참조값으로 빼고 결과 묶음에는 미리보기와 문서/데이터 정보만 둔다.
- 후속 질문에서 재사용할 source는 `current_data` 안에 작게 유지한다.
- route 값은 정해진 선택지처럼 다루고 오타는 정리 node에서 고친다.
...
일반적인 변환/조회 node에서는 실패 payload를 반환하는 편이 다음 node 처리에 유리하다.
...
## 11. 필요한 입력만 보여주는 field

입력값에 따라 다른 field를 보이거나 숨기면 초보 사용자도 화면에서 무엇을 입력해야 하는지 덜 헷갈린다. 예를 들어 인증 방식이 `none`이면 API key 입력칸을 숨기고, `bearer`이면 API key 입력칸을 보여준다.

구현할 때는 조건부로 보일 field를 `dynamic=True`로 표시하고, 기준이 되는 dropdown/toggle에는 `real_time_refresh=True`를 둔다. Langflow는 그 값이 바뀔 때 입력 화면을 다시 계산할 기회를 준다. 이 재계산 단계에서는 입력칸을 보이거나 숨기는 정도만 처리하고, 실제 API 호출이나 파일 처리는 실행 method에서 한다.
...
| --- | --- | --- |
| Input/Loader | 원문을 표준 결과 묶음으로 변환 | LLM 호출 |
| Prompt Builder | LLM에게 보낼 지시문 생성 | 모델 provider 호출 |
| LLM Caller | 지시문을 모델에 보내고 원문 응답 반환 | 복잡한 결과 모양 보정 |
| Parser | JSON/text를 dict로 읽기 | 업무 규칙 적용 |
| Normalizer | 별칭, 기본값, route, 결과 모양 보정 | 외부 DB 조회 |
| Router | 갈라질 길 선택 | 결과 묶음 대규모 변경 |
| Retriever | 외부 데이터 조회/검색 | 최종 자연어 답변 생성 |
| Merger | 여러 갈래/출처 결과 병합 | 새 조회 수행 |
| Analysis Executor | 제한된 계산 수행 | 임의 Python 전체 허용 |
| Answer Builder | 사용자용 답변 결과 생성 | 비밀값/점검 정보 노출 |
| Memory Writer | 다음 대화에 필요한 작은 상태 저장 | 전체 원본 행 무제한 저장 |

이 repo의 main flow도 이 사고방식에 맞춰 대화 상태, 관리 데이터, 의도 판단, 조회/검색, pandas 후처리, 최종 답변, 이전 대화 기억, API 응답을 분리한다.
...
| 구조 | 적합한 상황 | 장점 | 위험 |
| --- | --- | --- | --- |
| 고정 DAG | 조회 순서와 검증 기준이 명확한 업무 flow | 재현성, 확인 편의성, 감사 추적 | tool 선택 자유도가 낮음 |
| Tool Mode Agent | 작은 tool 여러 개 중 agent가 선택 | 구현 빠름, 자연스러운 tool 사용 | 호출이 불안정할 수 있음 |
| ReAct loop | 탐색, 반복 확인, 여러 tool 조합 필요 | 복잡한 문제 해결 가능 | 무한 반복, 비용, 확인 어려움 |

제조 데이터 분석처럼 "질문 -> 의도 판단 -> source 조회 -> 근거 기반 답변"이 중요한 경우는 고정 DAG를 기본으로 두는 것이 좋다. ReAct는 필요한 범위에만 제한적으로 도입한다.

ReAct를 직접 canvas로 만들 때 최소 안전장치:

- `max_steps`를 둔다.
- 같은 tool과 같은 입력값의 반복 호출을 막는다.
- 관찰 결과는 요약해서 넘긴다.
- tool 실패가 최종 답변에 그대로 새지 않도록 route를 정의한다.
- 마지막 답변은 반드시 근거 결과 묶음을 기준으로 만든다.

## 15. Agent 구현용 주요 node 모음
...
| --- | --- | --- | --- | --- |
| 입력/상태 | User Input Adapter, State Loader, Memory Extractor | 현재 질문과 이전 상태를 표준 상태로 변환 | `Message`, previous state, session id | `state_payload` |
| 관리 데이터 | Domain Loader, Table Catalog Loader, Filter Loader | 업무 규칙, 별칭, dataset, metric, filter 정의 로드 | JSON, MongoDB config, file | `domain_payload`, `table_catalog_payload` |
| Prompt | Intent Prompt Builder, Analysis Prompt Builder, Answer Prompt Builder | LLM에게 줄 입력을 목적별로 구성 | 상태, 관리 데이터, 근거 | `prompt_payload` |
| LLM 호출 | LLM JSON Caller, LLM Text Caller | provider/model/API key로 LLM 호출 | prompt, model config, secret | `llm_result` |
| 파싱/정리 | JSON Parser, Intent Normalizer, Analysis Plan Normalizer | LLM output을 안정적인 결과 모양으로 변환 | raw LLM text, 관리 데이터 | `intent_plan`, `analysis_plan` |
| 라우팅 | Request Router, Retrieval Router, Postprocess Router | 다음 갈래 결정 | plan, 조회 결과 | branch outputs |
| 조회 | File Retriever, Excel Dataset Loader, DB Retriever, HTTP API Retriever, Vector Retriever, MCP Gateway Caller | 외부 source에서 근거 확보 | query/job/source config | `source_result`, `retrieval_payload` |
| 비정형 문서 전처리 | Document Extractor, Multimodal Document To Text | 이미지/PDF/PPT를 text 근거로 변환 | file, instruction, vision model config | `document_text`, `document_summary` |
| 병합 | Source Result Merger, Branch Merger | 여러 source/갈래 결과를 하나로 통합 | source results | `retrieval_payload`, `analysis_result` |
| 분석 | Pandas Prompt Builder, Code Plan Normalizer, Pandas Executor | 조회 결과를 계산/집계/비교 | 조회 결과 묶음, code plan | `analysis_result` |
...
| 관측/검증 | Debug Payload Builder, Validation Result Builder | 개발자 확인용 상태 정리 | any payload | 작은 점검 정보/report |
...
| Metadata Loader | MongoDB/API 연결 실패는 `success=False`, 단 빈 관리 데이터로 계속 갈지 route를 멈출지 flow 정책으로 결정 |
| Prompt Builder | 필수 결과 묶음이 없으면 prompt를 만들지 말고 `errors` 반환 |
| LLM Caller | provider 오류, 제한 시간, 빈 응답을 `llm_debug`와 `errors`에 기록 |
| Parser | 원문 text와 읽기 오류를 남기고 뒤쪽 정리 node가 판단할 수 있게 함 |
| Normalizer | 가능한 기본값을 채우되, route/근거를 왜 보정했는지 `normalization_notes`에 기록 |
| Router | 선택되지 않은 갈래는 `success=False`, `active=False`, `skipped=True` 결과 묶음을 반환 |
| Retriever | 외부 source 오류를 전체 agent 오류와 분리해 source별 `error_message`로 기록 |
| Executor | 실행 실패 시 실패한 code/plan과 제한 사유를 debug에 넣되 비밀값/source credential은 제외 |
| Final Builder | 근거가 부족하면 추측 답변 대신 제한/확인 필요 답변을 생성 |

## 16. 주요 node 구현 패턴

여기서는 실제 agent 구현에 자주 쓰이는 핵심 node를 어떻게 구현해야 하는지 설명한다. 모든 코드를 완성형으로 길게 넣기보다, 개발자가 자기 업무에 맞게 확장할 수 있는 기본 틀과 결과 묶음 형식을 중심으로 본다.
...
- 사용자의 현재 질문은 `pending_user_question`처럼 뒤쪽 node가 찾기 쉬운 key에 둔다.
...
- 뒤쪽 merger가 active branch만 선택할 수 있게 한다.
...
- 뒤쪽 node가 pandas 분석을 해야 하는지 판단할 수 있게 `source_summaries`를 만든다.
...
- output payload key를 다음 node가 그대로 기대하는가?
...
### 17.5 교육 자료에 추가한 공통 노드 예시

사내 공통 component library로 올리기 전에 `examples/` 폴더의 예시를 먼저 Langflow 코드창에 붙여 넣어 단독 실행해 본다. 특히 현재 시간, route 분기, 문서/데이터셋 적재처럼 여러 flow에서 반복되는 부품은 작은 component로 분리해 두면 교육과 운영 모두 편해진다.

| 예시 파일 | Langflow node | 언제 쓰는가 | 핵심 output |
| --- | --- | --- | --- |
| `examples/current_time_context.py` | Current Time Context | "오늘", "이번 주", "현재 기준" 같은 상대 시간 질문을 prompt에 넣을 때 | `Time Context(Data)`, `Time Message(Message)` |
| `examples/smart_route_payload_builder.py` | Smart Route Payload Builder | Smart Router/Structured Output route 결정을 Route Gate payload로 정리할 때 | `Route Payload(Data)` |
| `examples/route_gate.py` | Route Gate | `data_retrieval`, `document_rag`, `time_answer`, `final_answer` branch를 동시에 노출할 때 | branch별 `Data` |
| `examples/file_dataset_loader.py` | File Dataset Loader | CSV/JSON/JSONL/Markdown/Excel 데이터셋을 표준 payload로 정리할 때 | `dataset(Data)` |
| `examples/pdf_page_image_extractor.py` | PDF Page Image Extractor | 이미지/스캔 PDF를 페이지 PNG로 분리해 vision model에 넣을 때 | `Vision Message(Message)`, `Page Manifest(Data)` |

아래의 Excel loader와 multimodal document node는 추가 확장 설계 예시다. 실제 구현은 위 `examples/`의 파일 처리/시간/분기 패턴을 참고해서 작성한다.
...
아래 node는 `FileInput`, `DropdownInput`, `BoolInput`, `IntInput`을 함께 사용하는 예시이다. CSV, JSON, JSONL, TXT, Markdown, Excel 파일을 읽고, 다음 node가 쓰기 쉬운 `Data` payload로 변환한다.

기본 `Read File`은 PDF/DOCX/PPTX처럼 문서를 읽어 텍스트나 markdown으로 넘기는 범용 노드에 가깝다. `File Dataset Loader`는 그 대체품이라기보다, 사내 데이터셋을 `row_count`, `columns`, `preview_rows`, `warnings`, `rows` 같은 동일한 결과 형식으로 맞춰 다음 component가 같은 방식으로 읽게 만드는 adapter 예시이다.
...
운영 단계에서는 큰 파일을 바로 `rows`에 담기보다 storage writer node를 붙여 `data_ref`를 만들고, 다음 node에는 preview와 ref만 넘긴다.
...
- 다음 node가 기대하는 payload key를 만든다.
...
Retriever
  ...
  -> data_ref 생성
  -> 다음 node에는 data_ref + preview + summary 전달
```
...
- output payload가 다음 node에서 읽히는가?
