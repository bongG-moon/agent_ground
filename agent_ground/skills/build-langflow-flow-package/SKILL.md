...
1. 대상 버전을 Langflow `1.9.2` / langflow-base `0.9.2` / LFX `0.4.2`로 고정하고 기존 Flow JSON의 실제 노드 schema를 확인한다.
...
- 기본 노드는 1.9.2의 실제 starter 또는 LFX 0.4.2 Component index에서 가져오고, Custom node는 원본 Python으로 template을 재생성한다.
- Flow의 `last_tested_version`과 manifest의 `source_export_version`은 `1.9.2`로 기록한다.
...
실제 Langflow가 실행 중이면 버전과 health를 먼저 확인한다. 설치 버전이 1.9.2가 아니면 프로젝트 전용 격리 환경에서 template 생성·Graph parse를 검증한다. 라이브 DB나 사용자의 기존 Flow에는 명시적 요청 없이 import하지 않는다.
