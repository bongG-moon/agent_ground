...
- `manifest.json`: ID, 한글 이름, 상태, version, 목적, 입력, 출력, 위험, 검증 환경, `source_export_version=1.9.2`와 문서 경로
...
- 모든 실행 node가 LFX 0.4.2 loader에서 재생성 또는 로드되고, 개별 JSON과 bundle이 1.9.2 Graph parser에서 해석되어야 한다.
