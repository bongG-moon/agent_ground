...
1. 관련 Flow, 기존 유사 컴포넌트를 확인하고 대상 버전을 Langflow `1.9.2` / langflow-base `0.9.2` / LFX `0.4.2`로 고정한다.
...
- `create_component_template({"code": code, "output_types": []})`를 LFX `0.4.2`에서 실행해 입력·출력, code hash와 dependency metadata를 다시 만든다.
- 화면 타입은 1.9.2 기준 `JSON`·`Table`을 우선 사용한다. Python 구현에서 `Data`·`DataFrame` 클래스를 쓰면 화면 이름과 내부 클래스 이름이 다른 이유를 설명한다.
- `LANGFLOW_ALLOW_CUSTOM_COMPONENTS=false`인 운영 환경을 고려해 승인된 `LANGFLOW_COMPONENTS_PATH` 배포 방식 또는 관리 정책을 문서화한다.
...
해당 컴포넌트의 단위 테스트와 관련 Flow 회귀 테스트도 실행한다. 로컬 Langflow가 이미 실행 중이면 `/api/v1/version`과 `/health`를 읽기 전용으로 확인하되, 버전이 1.9.2가 아니면 대상 검증으로 간주하지 않는다. 사용자 요청 없이 라이브 Flow를 import하거나 덮어쓰지 않는다.
