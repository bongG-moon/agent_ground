...
- 모든 환경의 기본 대상은 Langflow `1.9.2`, langflow-base `0.9.2`, LFX `0.4.2`, Python `3.12`다.
- PC에 설치된 Langflow 버전이 다르면 프로젝트 전용 격리 환경을 만들고 그 환경에서만 Flow 생성기와 template 검사를 실행한다.
- `.venv`, Langflow 사용자 DB와 cache는 이동하지 않는다. 각 환경에서 같은 버전으로 다시 만든다.
- 버전 기준 변경은 한 환경에서 임의로 처리하지 않고 Master Guide·Skills·manifest·생성기·교육자료를 한 커밋에서 함께 바꾼다.
...
8. 병합 후 Flow JSON의 `last_tested_version`, manifest의 `source_export_version`, embedded custom metadata의 LFX version이 공통 실행 기준과 일치하는지 확인한다.
