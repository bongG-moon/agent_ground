...
3. 기본 대상 환경을 **Langflow 1.9.2 / langflow-base 0.9.2 / LFX 0.4.2 / Python 3.12**로 고정한다. 로컬에 다른 버전의 Langflow가 설치되어 있어도 생성·검증 기준으로 사용하지 않는다.
4. 기억이나 다른 저장소의 구조보다 현재 체크아웃의 파일과 계약을 우선한다.
5. 작업 종류에 맞는 Skill을 함께 적용한다.
...
6. 사용자가 요청하지 않은 라이브 Langflow import, DB 변경, 배포, 외부 발송은 수행하지 않는다.
...
- 신규·수정 Component와 Flow는 별도 지시가 없는 한 Langflow `1.9.2`, langflow-base `0.9.2`, LFX `0.4.2`의 실제 template과 Graph 계약으로 생성한다.
- 버전 기준을 바꾸려면 사용자의 명시적 요청과 격리 환경 검증이 필요하며, Master Guide·Skills·생성기·manifest·교육자료·회귀 테스트를 함께 갱신한다.
- Langflow 1.9 화면 용어는 `JSON`과 `Table`을 사용하되, Standalone Python 내부 호환 클래스명 `Data`와 `DataFrame`이 필요한 경우 그 차이를 문서에 설명한다.
...
생성 스크립트가 별도 `--check` 옵션을 제공하면 생성 후 해당 검사도 실행한다. Flow 생성기와 template 검사는 정확한 1.9.2 격리 Python에서 실행한다. 실제 Langflow 실행을 주장하려면 확인한 버전, 입력, 결과와 남은 수동 검증을 구분해 기록한다.
