...
2. 포털에 표시할 기본 환경이 Langflow `1.9.2` / LFX `0.4.2`인지 확인한다.
3. 생성 페이지인지 직접 관리하는 교육 본문인지 구분한다.
4. 생성 페이지는 생성기를 먼저 수정하고 HTML을 재생성한다.
5. 기존 교육 주제와 참고자료는 보존하되, 낡은 화면명·버전·동선은 1.9.2 기준으로 다시 설명하고 디자인과 배치를 개선한다.
6. 정적 `file://`로 열어도 핵심 탐색, 계약, 코드 보기가 동작하게 만든다.
...
- 교육자료 첫 화면에서 현재 기준 `Langflow 1.9.2 · LFX 0.4.2`와 `JSON/Table` 화면명 변경을 찾을 수 있게 한다.
- 1.8.2에서 발생한 문제 해결 기록은 역사적 기록으로 남길 수 있지만, 현재 구현 기준과 혼동되지 않게 `과거 1.8.2 기록`으로 표시하고 1.9.2 이관 안내에 연결한다.
