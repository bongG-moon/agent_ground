...
def _clone_node(prototype: dict[str, Any], node_id: str, position: tuple[float, float]) -> dict[str, Any]:
    ...
    if isinstance(node.get("data", {}).get("node"), dict):
        node["data"]["node"]["lf_version"] = LANGFLOW_VERSION
    return node
...
def _configure_file_node(node: dict[str, Any]) -> None:
    ...
    if output.get("name") != "dataframe" or "Table" not in output.get("types", []):
        raise ValueError("04 파일 읽기 Custom Component의 1.9.2 출력은 Table이어야 합니다.")
...
def build_flow(*, use_dummy_source: bool = False) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    prompt_donor = _find_first_node_by_type(document_starter, "Prompt")
    loop_donor = _find_first_node_by_type(loop_starter, "LoopComponent")
    model_donor = _find_first_node_by_type(loop_starter, "LanguageModelComponent")
    chat_input_donor = _find_first_node_by_type(image_starter, "ChatInput")
    chat_output_donor = _find_first_node_by_type(image_starter, "ChatOutput")
    note_donor = _find_first_node_by_type(loop_starter, "note")
    ...
    flow: dict[str, Any] = {
        ...
        "description": (
            "Langflow 1.9.2 test flow that creates realistic local EWS mail rows without network access."
            if use_dummy_source
            else (
...
def validate_flow(flow: dict[str, Any], sources: dict[str, str], *, use_dummy_source: bool = False) -> None:
    ...
    if use_dummy_source:
        ...
        if dummy_template["include_vision_attachment"].get("value") is not True:
            raise ValueError("더미 Flow는 기본 JPG Vision 예제를 포함해야 합니다.")
    else:
    ...
    if "Table" not in file_to_formatter["data"]["sourceHandle"].get("output_types", []):
        raise ValueError("04 Read File의 1.9.2 출력은 Table이어야 합니다.")
    if "Table" not in file_to_formatter["data"]["targetHandle"].get("inputTypes", []):
        raise ValueError("05 메일 내용 정리의 1.9.2 입력은 Table 단일 타입이어야 합니다.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the Langflow 1.9.2 mail attachment summary flow.")
