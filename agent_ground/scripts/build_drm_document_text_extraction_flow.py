...
def build_flow() -> tuple[dict[str, Any], str]:
    ...
    wrapper = _find_first_node_by_type(starter, "ChatInput")
    ...
    flow: dict[str, Any] = {
        ...
        "description": (
            "Langflow 1.9.2 flow that extracts plain files locally, sends protected files "
...
def validate_flow(flow: dict[str, Any], source: str) -> None:
    ...
    for key in ("sourceHandle", "targetHandle"):
        ...
        if edge[key] != _handle_text(edge["data"][key]):
            raise ValueError(f"Langflow 1.9.2 handle 형식이 아닙니다: {key}")
