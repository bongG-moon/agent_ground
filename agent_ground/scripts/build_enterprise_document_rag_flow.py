...
try:
    ...
    from scripts.langflow_1_9_2_compat import (
        TARGET_LANGFLOW_VERSION,
        TARGET_LFX_VERSION,
        assert_target_runtime,
    )
...
FLOW_SOURCES = (
    ...
    / "drm_document_text_extraction_flow.json",
...
def _find_first_node_by_type(flow: dict[str, Any], node_type: str) -> dict[str, Any]:
    for node in flow.get("data", {}).get("nodes", []):
        if node.get("data", {}).get("type") == node_type:
            return node
    raise KeyError(f"Donor node type not found: {node_type}")
...
def _handle_text(value: dict[str, Any]) -> str:
    """Langflow 1.9.2 프런트엔드의 안정적인 JSON handle 인코딩을 사용합니다."""
...
def build_flow() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    flow: dict[str, Any] = {
        ...
        "description": (
            "Standalone Langflow 1.9.2 enterprise document RAG example with document normalization, "
    ...
    for edge in edges:
        ...
        for key in ("sourceHandle", "targetHandle"):
            ...
            if edge[key] != _handle_text(edge["data"][key]):
                raise ValueError(f"Handle is not in stable Langflow 1.9.2 form: {edge.get('id')} {key}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the Langflow 1.9.2 enterprise document RAG flow export.")
