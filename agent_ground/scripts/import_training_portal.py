...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="과거 교육 포털을 참고용으로 다시 가져오는 일회성 도구입니다."
    )
    parser.add_argument(
        "--allow-legacy-overwrite",
        action="store_true",
        help="현재 1.9.2 교육자료를 과거 원본으로 덮어쓰는 위험을 이해한 경우에만 사용합니다.",
    )
    args = parser.parse_args()
    if not args.allow_legacy_overwrite:
        raise SystemExit(
            "현재 교육자료는 Langflow 1.9.2 기준으로 직접 관리합니다. "
            "과거 원본으로 되돌려야 하는 명시적 작업에서만 --allow-legacy-overwrite를 사용하세요."
        )
    ...
    if guide.is_file() and not (references / guide.name).exists():
