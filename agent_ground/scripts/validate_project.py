...
IGNORED_TOP_LEVEL_DIRS = {
    ".git",
    ".langflow-1.9.2-config",
    ".langflow-1.9.2-source",
    ".langflow-1.9.2-venv",
    ".uv-cache",
    ".pytest_cache",
    ".pytest-tmp",
...
RUNTIME_HTML_OUTPUT_DIRS = {
    ROOT / "flows" / "html_report_flow" / "report_api" / "storage",
}
...
def json_files() -> list[Path]:
    return sorted(
        ...
        if not any(part in IGNORED_TOP_LEVEL_DIRS for part in path.relative_to(ROOT).parts)
    )
...
def validate_html() -> tuple[int, int]:
    ...
    outside = [
        ...
        if HTML_ROOT not in path.parents
        and not any(part in IGNORED_TOP_LEVEL_DIRS for part in path.relative_to(ROOT).parts)
        and not any(runtime_dir in path.parents for runtime_dir in RUNTIME_HTML_OUTPUT_DIRS)
    ]
    if outside:
    ...
    for edge in individual["data"]["edges"]:
        for handle_name in ("sourceHandle", "targetHandle"):
            ...
            if "┇" in encoded or "œ" not in encoded:
                raise AssertionError(f"Invalid Langflow 1.9.2 handle delimiter: {edge.get('id')}")
...
def validate_training_preservation() -> dict[str, int | bool]:
    ...
    heading_cursor = iter(target_audit.headings)
    for source_heading in source_audit.headings:
        if not any(candidate == source_heading for candidate in heading_cursor):
            raise AssertionError(
                f"기존 교육 주제의 순서가 보존되지 않았습니다: {source_heading}"
            )
    ...
    missing_markers = [marker for marker in required_1_9_2_markers if marker not in target]
    if missing_markers:
        raise AssertionError(f"교육자료의 1.9.2 기준 설명이 누락되었습니다: {missing_markers}")
    return {
        "source_lines": len(source.splitlines()),
        "headings": len(source_audit.headings),
        "references": len(source_audit.references),
        "content_topics_preserved": True,
