...
> 상태: `user_testing`  
> 기준 환경: Langflow `1.9.2`, LFX `0.4.2`  
...
Langflow `1.9.2`의 shared component cache는 backend process 메모리에 있으며 기본 만료시간은 1시간입니다. 서버를 재시작하면 사라지고 여러 worker가 같은 cache를 공유하지 않습니다. Shared component cache가 없는 환경에서는 캐시 없이 실행을 계속합니다.
...
이 Component는 Langflow `1.9.2` / LFX `0.4.2`의 내부 `RunFlowBaseComponent`, `~` tweak separator와 shared component cache 계약을 사용합니다. Langflow를 업그레이드할 때는 다음을 다시 확인해야 합니다.
