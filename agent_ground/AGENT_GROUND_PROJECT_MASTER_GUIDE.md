...
> 문서 상태: 초기 기준안 1.0  
> 작성일: 2026-07-10  
> 적용 대상: `C:\Users\<사용자명>\Desktop\Agent_ground\agent_ground`  
...
> 기본 대상 버전: **Langflow 1.9.2 / langflow-base 0.9.2 / LFX 0.4.2 / Python 3.12**. 별도 지시가 없으면 신규·수정 자산과 교육자료는 이 기준을 사용한다.
...
- 이 프로젝트의 기준 Agent Builder인 Langflow 1.9.2에서 보이는 노드, 입력 필드, 타입, 패키지, 제약을 우선한다.
- 구현자는 로컬 검증을 하고, 사용자는 실제 환경에서 직접 실행해 확인한다.
- 문서에는 마지막으로 확인한 환경과 날짜를 표시한다.
- 확인하지 않은 내용은 `검증 완료`처럼 표현하지 않고 `사용자 환경 확인 필요`로 표시한다.
- 로컬 설치 버전이 1.9.2가 아니면 프로젝트 전용 격리 환경을 사용하며, 다른 버전에서 통과한 결과를 1.9.2 검증으로 간주하지 않는다.
- 1.9 화면 용어인 `JSON`과 `Table`을 우선 사용하되 Python 호환 클래스명 `Data`와 `DataFrame`이 필요한 경우 둘의 차이를 설명한다.
...
12. **Pinned runtime**: 별도 지시가 없으면 Langflow 1.9.2 / langflow-base 0.9.2 / LFX 0.4.2를 사용한다. 기준 변경은 격리 검증 후 Master Guide, Skills, 생성기, manifest, 교육자료와 테스트를 함께 수정한다.
...
```text
Agent_ground/
├─ README.md                     # 저장소 내 프로젝트 구분
├─ agent_skill_hub/              # 별도 Skill Hub 프로젝트
├─ deliverables/                 # 최종 전달 산출물
│
└─ agent_ground/
   ├─ README.md
   ├─ FOLDER_GUIDE.md
...
> 2026-07-26 구현 상태: Langflow 1.9.2 / LFX 0.4.2 template으로 재생성한 24-node / 34-edge 실행 Flow, 공용 Component Library와 분리된 메인 10개·운영자용 5개 Standalone 실행 Node, graph Normalizer, 분기형 HTML Renderer와 Import Bundle까지 구현했으며 현재 `user_testing`이다. 추천 정책은 계속 `status=approved` 자산만 허용한다.
...
- 실제 Langflow `1.9.2` / LFX `0.4.2` template·Standalone loader·Flow Graph parse와 격리 계약 테스트는 통과했지만, 사내 endpoint·계정 호출과 GooDocs 실제 모듈 교체, Datalake MySQL driver 설치는 사용자 환경에서 확인해야 한다.
