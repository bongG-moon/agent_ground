...
- 공개 Component 21개, Flow 내부 Node 33개, Business Agent Design 전용 Node 15개 등 Standalone 원본 69개를 LFX 0.4.2 loader로 재평가
- 10개 Flow JSON의 132 nodes / 145 edges를 Langflow 1.9.2 Graph parser로 검증
- built-in·custom Node template, `lf_version`, code metadata와 `œ` edge handle을 1.9.2 계약으로 다시 생성
- `Data→JSON`, `DataFrame→Table` 화면 alias와 통합 Language Model 선택기를 Flow·테스트·교육자료에 반영
- `mail_attachment_summary_flow`를 14 nodes / 12 edges로 갱신하고 Table 기반 Reader·Formatter 연결 계약 반영
- Project Bundle에 DRM 문서 추출 Flow가 빠지지 않도록 모든 생성기의 7개 Flow 순서를 통일
- 전체 회귀 테스트 109개, Project 구조·포털 128페이지·로컬 링크 2,886개 검증 통과
- 1.9.2 이관 가이드, 환경 프로필, Master Guide, 이동식 Skill 4종과 현재 검증 보고서 갱신
- 저장소 최상위를 `agent_ground/`, `agent_skill_hub/`, `deliverables/`로 분리하고 과거 PPT 임시 작업 폴더 `work/` 삭제
- `FOLDER_GUIDE.md`를 추가해 Component, Flow, `scripts/`, registry, training, skills와 archive의 책임을 명시
