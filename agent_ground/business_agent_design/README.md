...
1. JSON을 Langflow 1.9.2에 Import합니다.
...
생성기는 Langflow 1.9.2 프런트엔드 계약에 맞춰 edge handle 문자열의 quote delimiter로 `œ`를 사용하고, 각 문자열을 `JSON.parse(text.replaceAll("œ", '"'))`와 같은 방식으로 다시 해석할 수 있는지 검사합니다. 모든 Standalone node는 LFX 0.4.2 원본 loader로 template을 다시 만들며, 오류가 났던 구분자 `┇`는 허용하지 않습니다.
