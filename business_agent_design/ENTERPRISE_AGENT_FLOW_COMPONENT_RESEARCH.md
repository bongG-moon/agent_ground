...
1. 12개 데이터 Component를 실제로 연결한 Langflow 1.8.2 JSON 재생성
2. manifest, `component_refs.json`, JSON node class와 port 자동 대조
3. 대표 자연어 요청으로 Oracle/H-API/Datalake/Goodocs branch 확인
4. Project Bundle 재생성

### 8.2 데이터 Source Component가 항상 dummy 결과를 반환

현재 4개 Source Component에는 `_dummy_rows`가 항상 존재하고 실행 함수가 `callable(dummy_builder)`를 확인한다. 따라서 실제 연결 설정을 넣어도 dummy branch가 먼저 실행된다. Oracle의 실제 실행 코드는 주석 상태이고 다른 Source도 같은 패턴이다.
...
- 자동 기능 테스트는 Business Agent Design 영역에 집중돼 있다. 공용 21개 Component에도 정상·빈 입력·인증 실패·timeout 테스트가 필요하다.
