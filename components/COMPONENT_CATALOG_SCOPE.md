...
이 감사 기준에 따라 기존 41개 폴더를 Component 19개와 Flow 내부 node 22개로 재분류했다. 이후 PPT 참고 이미지 기반 HTML 프레젠테이션 기능을 추가하면서 독립 Renderer 1개와 Flow 전용 node 6개가 늘었다.

- **Component 20개**: `components/<component_id>/`에 유지
- **Flow 내부 node 28개**: `flows/<flow_id>/nodes/`로 이동
...
| `ppt_reference_html_flow` | 6 | `flows/ppt_reference_html_flow/nodes/` |
...
Registry에는 다음 25개 자산만 들어간다.

- 자격을 통과한 Component 20개
- 최상위 Flow 5개

Flow 내부 node 28개는 Registry 추천 자산이 아니다. Business Agent Design도 Registry에 있는 20개 Component와 5개 Flow만 추천 대상으로 사용해야 한다.
