...
- 전체 Standalone Component inventory는 43개다.
- F10의 각 보완 회차는 `12 완전성 평가 → 질문 LLM → 13 질문 Batch → 42 선택형 답변 카드 → 41 번호형 안내 → 49/47/46/39 답변 재개·반영`으로 보인다. Component 42는 choice-only native `node_input` pause를 보내며, 사용자는 답변 입력하기·skip·Cancel 중 하나만 고른다. `1번: 답변` 메시지는 정확한 질문 번호와 권위 batch를 검증한 뒤 제출되며, skip은 audit·`unresolved`를 기록한 뒤 review로 간다. 1·2차는 최대 3문항, 마지막 3차는 최대 4문항이며 skip은 4차 질문이 아니다. `48`은 다음 2차 또는 3차 질문 하나만 열고, 검토 가능한 결과와 skip audit은 `39`가 `40`에 직접 보낸다. `40`은 검토 진입 중 유효한 하나를 합치며, `43`은 최종 Approve/Reject/Cancel 중 선택되지 않은 두 상태 저장 branch를 제외하고, `41`은 선택된 안내 또는 terminal event-list만 안전하게 표시한다.
...
- F10 시작 Text Input 네 개(팀 명·업무 설명·사번·추가 설계 프롬프트)는 `samples/f10_work_request_example.json`의 안전한 데모 값으로 채워져 있으며 Component 원본의 범용 기본값은 비어 있다. 사번 Text Input은 초기 요청과 질문 답변 재개에 공통으로 연결되므로 같은 실행에서는 변경하지 않는다.
...
| Standalone source build | `43/43` build 및 manifest inventory 일치 |
| F00 visible vector pipeline | 전체 snapshot 명시 확인, deterministic ID, partial resume, CAS 활성화, provider 호출 간격 회귀를 포함해 통과 |
| F10 미빌드 sibling 방지 | planner/batch/HITL/chat loader/parser/commit/router/review/store/auth/loader의 blocked·cancel·reject·답변 안내 terminal 경로를 Component 41 event-list 입력으로만 표시하며 선택되지 않은 sibling build를 요구하지 않음 |
...
| Runtime validator | target `langflow=1.11.1`, `langflow-base=0.11.5`, `lfx=1.11.5`에서 `ok=true`, Flow 5개·Standalone Component 43개 확인 |
...
- Component subclass 수와 전체 inventory 43개 일치
...
- F10 native HITL의 `42` choice-only pause와 빈 branch payload, 답변 입력하기/Skip/Cancel route 상호 배타 판정, `49` 첫 실행·번호형 답변 분기, `47` 권위 batch 재조회, `46` 번호·필수값·선택값 parser, `48` 다음 단계 단일 선택, skip audit/unresolved/unknown provenance/review projection, Component `43`의 final branch conditional exclusion, Component `41` event-list terminal/번호형 안내 projection, WorkDefinition revision/state, Component 36 authority reload, F10→F20→F30 direct Run Flow, Skill/검색 ACL, blueprint/readiness, report CSP/XSS 회귀 범위
...
정적 build·unit contract 통과만으로 Playground 화면의 실제 pause/resume 동작까지 증명되지는 않는다. 실제 Langflow 1.11.0/1.11.1 import 후 질문이 발생하는 예제로 F10을 실행해, `42 보완 답변 HITL` 카드에 **답변 입력하기** / **추가 입력 건너뛰기** / **Cancel**만 보이는지, 답변 입력하기 뒤 Component 41이 복사 가능한 `1번:` 양식을 표시하는지, 같은 Chat Input의 번호형 답변이 `49 → 47 → 46 → 39`으로 반영되는지, skip이 answer value를 만들지 않고 audit/unresolved 뒤 review로 가는지를 별도 E2E로 확인해야 한다. 이 확인 전에는 외부 Answer Form/API가 필요 없다는 UI 동작을 production 승인을 위한 완료 증적으로 취급하지 않는다.

생성 manifest의 Flow별 결과:

| Flow | Canvas / 실행 / Note / Edge | SHA-256 |
| --- | ---: | --- |
| F00 | 8 / 6 / 2 / 5 | `5d09918c229777623bac750a6003fa38715e93cc9cd3b588253d8e22af635d6b` |
| F10 | 53 / 47 / 6 / 126 | `3b85a6dfa4a977229f542dd11c26bf259f907d35067bf581ad8f61cd586ef192` |
| F20 | 23 / 19 / 4 / 27 | `183ab56593d0e7e3f1c2d04aaa204d0910e6b8f457bd006a250e664719da9ceb` |
| F30 | 9 / 8 / 1 / 9 | `3e7079fa910d91e4fa10f727c715d1101bee25280d98a098ce26bf9f0107b2f4` |
| F90 | 11 / 9 / 2 / 9 | `9f7b1f5cbba60a95f22cab6d4592f57f4c2bc67827a4fc87b064229e7385dc7c` |
...
- 실제 Langflow project에서 F10의 새 업무 시작·choice-only native HITL·번호형 Chat Input 재개와 최대 3회 질문(1·2차 최대 3문항, 3차 최대 4문항) 및 explicit skip E2E
