...
- 43개 Custom Component: 모두 한 `.py` 파일에 완결된 Standalone Component
...
- `samples/f10_work_request_example.json`: F10의 네 Text Input(팀 명·업무 설명 원문·사번·추가 설계 프롬프트)에 들어 있는 **생산·프로젝트 리스크 통합 주간보고** 복합 예제(메일·CUBE·JIRA·DataLake·SOP·승인·예외 처리 포함)
...
F10 Canvas는 Sticky Note 6개와 함께 정리했습니다. 처음 시작할 때는 Canvas의 네 Text Input에서 팀 명·업무 설명 원문·사번·추가 설계 프롬프트를 확인·수정하고, Playground 채팅 입력을 비워 두거나 `새 업무 시작`이라고 보냅니다. 사번은 질문을 받는 사람을 찾는 기준이므로 **첫 실행 전에 한 번 정하고 답변을 보낼 때까지 바꾸지 않습니다.** `49_f10_playground_entry_router.py`가 이 첫 실행을 기존 업무 추출 경로로 보냅니다. 세 번의 질문·답변·재평가와 최종 승인은 그대로 유지합니다. 다만 운영 Langflow 1.11.0의 HITL 카드는 **선택 버튼만** 제공하므로, `42_f10_clarification_answer_gate.py`에서는 **답변 입력하기** / **추가 입력 건너뛰기** / **Cancel**만 고릅니다. 답변 입력하기를 선택하면 `41_f10_terminal_result_message.py`가 `[질문과 입력 안내]`와 오염 없이 복사할 수 있는 `[복사용 답변 양식]`을 보여 줍니다. 사용자는 양식의 `1번: 답변`, `2번: 답변`처럼 채운 뒤 같은 Playground 채팅창에 전송하면 됩니다. `49`가 이 메시지를 답변 재개 경로로 보내고, `47_f10_chat_answer_resume_loader.py`가 **같은 사번**에게 열린 질문 묶음을 MongoDB에서 안전하게 찾습니다. 이어서 `46_f10_numbered_chat_answer_parser.py`가 번호·필수 여부·답변 형식을 검증한 뒤 `39_f10_answer_commit.py`에 전달합니다. Component 39는 답변을 `clarification_batches`에 기록·검증하고 병합·revision CAS 저장 후 다음 회차·검토·차단 중 하나만 엽니다. 별도 Answer Form이나 Flow 간 HTTP API는 필요하지 않습니다. 질문 묶음이 하나만 열렸다면 `질문 묶음: qb-...` 줄은 생략해도 되고, 여러 건이면 안내문에 나온 ID를 맨 위에 붙여 구분합니다. 첫 질문이 실제로 필요한 경우 `13_clarification_batch_builder.py`가 WorkDefinition revision 0과 immutable batch를 멱등적으로 준비합니다. 검토 단계의 Component 18 `review_and_request_approval`은 검증된 Preview를 한 번의 CAS 저장과 event 기록으로 `WAITING_APPROVAL` 상태로 전환합니다. Revision·중복 실행 방지 키·컬렉션은 자동/내부값이고 MongoDB URI·Database만 환경 설정입니다. `40_f10_review_entry_joiner.py`는 가능한 검토 진입 결과 하나만 고르고, Component 12·13·16·17·18·39·40·42·46·47·48·49는 선택된 group output만 열어 실패가 후속 LLM·Human Input·저장 단계로 흐르지 않게 합니다. 최종 built-in Human Input 뒤의 `43_f10_final_approval_route_gate.py`는 선택하지 않은 두 상태 저장 branch를 즉시 제외합니다. 승인 성공 경로에서는 `45_f10_authentication_context.py`가 local demo fixture와 trusted gateway 인증을 구분하고, `36_approved_design_invocation_loader.py`가 sealed context의 subject와 WorkDefinition, 활성 catalog pointer, 승인 Skill registry를 MongoDB에서 다시 대조한 뒤 F20에 전달할 단일 invocation을 만듭니다.
