...
- `F10_work_definition_parent.json`: 선택형 native `node_input` 보완 카드와 번호형 Playground 채팅 답변, 최종 승인 후 F20→F30 Run Flow를 연속 실행하는 top-level 업무 정의 Flow
...
| F10 | 6 | 업무 추출·선택형 질문 카드와 번호형 채팅 답변으로 최대 3회 HITL 보완, 검토·승인, F20→F30 direct 실행 |
...
F10은 top-level native pause/resume을 사용한다. F00 Writer의 부분 적재 `node_input` Continue/Stop 카드는 **Langflow durable background job**에서만 표시된다. 일반 Canvas **Run Flow**는 durable job을 만들지 않으므로 F00은 checkpoint를 저장한 뒤 `PARTIAL_EMBEDDINGS_SAVED`와 `hitl.reason=durable_background_job_required`를 반환하며, 같은 파일·모델 설정의 새 실행으로 이어간다. F10에는 하나의 일반 Playground Chat Input이 있다. 첫 실행 전 Canvas의 팀 명·업무 설명·사번·추가 설계 프롬프트를 채우고, Chat Input을 비워 두거나 `새 업무 시작`을 입력하면 Component 49가 기존 Text Input 업무 추출 경로를 시작한다. 사번은 답변 batch를 찾는 기준이므로 이 실행의 번호형 답변을 모두 마칠 때까지 바꾸지 않는다. F10의 보완 질문은 Component 42가 만드는 **선택형** `node_input` 카드다. 운영 Langflow 1.11.0에는 질문별 동적 입력칸이 없으므로 사용자는 카드에서 **답변 입력하기**를 고른 뒤, `[복사용 답변 양식]`의 `1번: ...`을 같은 Chat Input에 보낸다. Component 49가 이 메시지를 Component 47→46→39 답변 재개 경로로 보내 같은 사번의 열린 질문 묶음을 읽고 번호형 답변을 반영한다. built-in `Human Input`은 F10 최종 `Approve`/`Reject`/`Cancel` 승인 단계 하나뿐이다. F00에는 built-in Human Input node를 추가하지 않으며, F20/F30/F90은 HITL-free child/evaluation Flow다.
...
F10은 Sticky Note 6개를 포함한 compact 상위 Flow다. 시작의 `Text Input` 네 개에서 팀 명·업무 설명 원문·사번·추가 설계 프롬프트를 각각 받고, `10 업무 요청 Envelope`는 이 값을 자동으로 받는다. 업무 추출 뒤 보완은 최대 3회이며 기본 경로는 `12 완전성 평가 → 질문 LLM → 13 재질문 Batch → 42 보완 답변 안내 HITL`이다. **사람이 사용하는 순서**는 다음과 같다.

1. 새 업무를 시작할 때는 Canvas의 팀 명·업무 설명 원문·사번·추가 설계 프롬프트를 먼저 확인한다. 사번은 질문을 받은 사람을 식별하므로 답변 완료 전까지 바꾸지 않는다. 그 후 Playground 채팅 입력을 비워 두거나 `새 업무 시작`을 보낸다. `49 Playground 진입 Router`가 네 Text Input을 사용하는 기존 시작 경로를 연다.
2. 질문 카드에서 질문을 읽고 **답변 입력하기**를 선택한다.
3. 화면에 나온 `질문 묶음: qb-...`와 `1번: [답변]` 양식을 복사한다.
4. 같은 Playground의 일반 채팅 입력창에 `1번: ...`, `2번: ...`처럼 각 답을 채워 한 번에 전송한다. 동시에 여러 질문 묶음이 열려 있을 때만 `질문 묶음: qb-...` 줄을 남긴다.
5. `49`가 답변으로 판별한 메시지를 `47 채팅 답변 재개 Loader`로 보내고, Loader가 권위 MongoDB에서 **같은 사번**의 대기 질문 묶음을 확인한다. `46 번호형 대화 답변 Parser`가 번호·필수값·선택값을 검증한 뒤 `39 답변 반영·다음 단계`가 저장·재평가한다.

질문 카드 자체에는 동적 입력칸이나 `Submit Answers` 버튼이 없다. 이는 운영 Langflow 1.11.0 Human Input 계약이 선택 버튼만 지원하기 때문이다. `추가 입력 건너뛰기`는 답을 추측하지 않고 미확정 항목과 audit을 남긴 뒤 검토로 가며, `Cancel`은 작성을 종료한다. 1차 질문이 실제로 필요한 경우에만 Component 13이 질문 batch와 동일한 identity의 revision 0 WorkDefinition을 내부적으로 idempotent하게 준비한다. 세 번째 답변 뒤에도 blocking gap이 남으면 `CLARIFICATION_ROUND_LIMIT`으로 차단하며 네 번째 질문 카드나 4차 질문 회차는 만들지 않는다. `48 채팅 답변 다음 단계`는 답변 반영 결과 중 2차·3차 질문으로 이어질 항목만 하나 선택한다. 검토 가능한 결과와 skip audit 결과는 `39 답변 반영·다음 단계`에서 `40 검토 진입 Joiner`로 직접 보내므로, 빈 48 출력이 검토를 가로막지 않는다. `40 검토 진입 Joiner`는 초기/질문 전후/답변 후의 10개 review entry 중 유효한 성공 결과 하나만 골라 검토로 보낸다. 검토 단계의 Component 18 `review_and_request_approval`은 Preview 검증본을 저장하면서 `WAITING_APPROVAL`로 전환한다. Revision·중복 실행 방지 키·컬렉션은 자동/내부값이며 MongoDB URI는 자동 연결된 `MONGO_URL`, Database만 환경 설정이다. built-in Human Input의 최종 Approve, Reject, Cancel은 `43 최종 승인 경로 Gate`를 거쳐 하나의 Component 18 상태 명령만 열며, 선택하지 않은 상태 저장 node는 즉시 제외된다. `41 F10 안내·결과 메시지`는 `[질문과 입력 안내]`와 오염 없이 복사할 수 있는 `[복사용 답변 양식]` 또는 intentional cancel/reject/blocked terminal 결과 하나를 민감정보 없이 표시한다. 승인 성공 경로의 `45 F10 인증 Context 경계`는 local demo fixture와 production trusted gateway identity를 구분하며, Component 36은 이 sealed context만 받아 canonical owner와 대조한다.

`14 Work Answer Loader`, `15 Work Answer Merger`, `34 Work Runtime State Store`, `35 Result Gate`, Answer Form/HITL API 연동, F11/Playground 분리 Flow, 4차 질문 회차는 현재 F10 Canvas 경로가 아니다. 14·15·34·35와 Answer Form/HITL API는 독립 검증 또는 과거 재사용을 위한 historical source/연동으로만 남아 있으며, 현재 실행 경로는 Component 49·42·47·46·39·48·40·41·43의 compact 계약을 사용한다.

`samples/f10_work_request_example.json`의 팀 명·업무 설명·사번·추가 프롬프트는 시작 Text Input 네 개의 로컬 데모 기본값으로 들어 있다. `session_id` 입력란은 없으며 Component 10이 Langflow 실행 session을 사용한다. 현재 공용 catalog/Skill 영역은 내부 `default` scope이므로 팀 명은 표시·감사 메타데이터다. production에서는 팀 명·사번과 Component 36 인증 주체를 trusted gateway가 주입하고, 팀별 격리가 필요할 때만 catalog/Skill scope도 함께 분리한다.

F10의 승인 성공 경로는 `36 Approved Design Invocation Loader → built-in TypeConverter → Run Flow` 순서다. Component 36은 승인 receipt를 MongoDB canonical WorkDefinition, active catalog pointer, active immutable Skill registry와 다시 대조한다. 첫 실행의 업무 요청 Envelope가 현재 경로에 있으면 이를 검증하고, 번호형 채팅 답변 후 재개 경로처럼 Envelope가 없는 경우에는 **승인 canonical WorkDefinition에 보존된 `f10_design_context`만** 복원해 사용한다. 새 채팅 문장은 설계 prompt나 검색 원문으로 사용하지 않는다. 인증 gateway가 주입한 subject가 업무 owner와 일치하고 canonical/request channel이 모두 정확히 `native_hitl`일 때만 `agent-design-invocation/v1`을 만든다. TypeConverter가 invocation의 strict JSON `text`를 Message로 바꾼 뒤 **운영 Langflow 1.11.0 호환** `Run Flow` direct mode의 F20 입력으로 전달한다. HTTP API나 별도 F20 수동 실행은 사용하지 않으며 reject/cancel/blocked 경로는 F20을 호출하지 않는다. F20 Flow의 UUID가 import 과정에서 유지되어야 하며, remap된 경우 같은 프로젝트/폴더에서 Run Flow node의 F20 선택을 한 번 갱신한다.

F20이 만드는 새 Standalone Component 생성 요청은 운영 Langflow 1.11.0 호환을 기준으로 한다. 이 export를 만드는 source/template 검증은 `langflow==1.11.1`에서 계속 수행하지만, 요청 본문의 운영 target을 1.11.1로 바꾸지 않는다. 과거에 hash로 봉인된 1.11.1 요청은 F30 보고서 표시용 legacy 입력으로만 허용하며 수정·재발행하지 않는다.
