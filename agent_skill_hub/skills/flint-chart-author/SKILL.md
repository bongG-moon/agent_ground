...
For MCP clients, run a version-pinned executable installed in advance from the company-approved internal registry or mirror:

```bash
flint-chart-mcp
```

Do not use `npx -y`, a public runtime download, or a remotely hosted MCP server. Keep the MCP process local or on an approved internal endpoint, and never send chart data outside the company boundary.

For JavaScript or TypeScript projects, install Flint through the approved internal package mirror and add only the
renderer peer dependencies needed by the backend you will render:

```bash
npm install --offline flint-chart
npm install --offline vega vega-lite vega-embed  # browser Vega-Lite rendering
npm install --offline echarts                    # ECharts rendering
npm install --offline chart.js                   # Chart.js rendering
```
