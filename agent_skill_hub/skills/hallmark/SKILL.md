...
**Enterprise boundary:** Study only public URLs through the approved read-only retrieval path, without internal identifiers in the request. Never fetch an internal, authenticated, or private-network URL. For those targets, use an attached screenshot or local HTML/CSS. Do not call external AI image services; use supplied or locally approved assets.
...
- [`assets.md`](references/assets.md) — load only when an enrichment archetype needs an approved local or company-hosted asset.
...
**The enrichment hierarchy is non-negotiable.** Reach for the highest approved tier you can ship: typography only → Tier A pure CSS art → Tier B hand-built SVG → Tier C user-supplied or company-approved local still → Tier D approved vendored library asset → **Tier E local Lottie is last resort**, only for complex character motion that hand-build can't reach. Reaching for Lottie when CSS would have built it is the new tell.

When an enrichment archetype requires construction, also load [`custom-craft.md`](references/custom-craft.md). When it requires an asset, load [`assets.md`](references/assets.md) and enforce its enterprise-safe local asset rules.
