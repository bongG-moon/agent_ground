...
| **C · Approved local still** | User-supplied or company-approved image, with provenance and local post-processing. | Characters or specific scenes that hand-build cannot economically reach. |
| **D · Vendored illustration** | Licence-approved illustration committed locally and customised with brand colours. | When a pre-approved library asset is already available. |
| **E · Vendored Lottie animation** | LAST RESORT. Approved JSON and renderer kept locally. | Articulated figures or multi-frame mascot loops that cannot be hand-built. |
...
See [`custom-craft.md`](custom-craft.md) for *how* to build at tiers A and B. See [`assets.md`](assets.md) for the enterprise-safe rules at tiers C, D, and E.
