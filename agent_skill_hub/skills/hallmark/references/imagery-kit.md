...
- **Don't use kit imagery as the literal subject.** An abstract bottle is *not* a stand-in for an actual coffee-shop hero — use a clearly labelled local placeholder until approved photography is supplied. The kit is for atmosphere / composition / decoration, not subject replacement.
...
## Internal asset maintenance pipeline
...
**Tooling.** Use only user-supplied assets or company-approved internal creation tools. Never upload reference images, prompts, brand assets, or project context to an external generation service. Record provenance in the repository.
...
**Refresh.** Treat the kit as a maintainable batch. When the palette catalogue changes, update the relevant palette family with approved internal tools. The manifest above is the spec; locally committed assets are the deliverable.
