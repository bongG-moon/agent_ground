...
**Fix.** Hand-build the illustration in pure CSS or SVG (Tier A or B in [`hero-enrichment.md`](hero-enrichment.md)). Otherwise use only a user-supplied or company-approved local asset, with asymmetric crop and local post-processing. Never upload brand material to an external generator.
