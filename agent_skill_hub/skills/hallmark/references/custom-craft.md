...
## Tier E · Approved supplied stills

When characters or specific scenes are needed and hand-building is uneconomical, use only a user-supplied image or an asset produced by a company-approved internal tool. Never send prompts, reference images, brand material, or project context to an external generation service.

### Discipline for supplied stills

- Record the source, licence, approval status, and acquisition date.
- Post-process locally: crop, colour-grade, optimise, and verify accessibility.
- Inspect for visual defects and misleading details before use.
- Keep the original and the processed derivative according to project convention.
- Treat multi-frame motion as Tier F and use only approved local files.

---

## Tier F · Vendored illustrations + Lottie (last resort)

Use only when an approved illustration or Lottie file is already supplied through the company asset process. The enterprise-safe rules are in [`assets.md`](assets.md). Even at this tier:
...
The Lottie tell is a generic library pull where pure CSS would have built it stronger and lighter. The audit verb catches this.
