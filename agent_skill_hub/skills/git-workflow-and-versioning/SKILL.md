...
Keep repositories and remotes inside the company-approved Git service. Never push source, diffs, tags, logs, or artifacts to a public remote. A commit is a local action; pushing, tagging a remote, or opening a review requires an explicit user request and an approved internal destination.
...
git push <approved-internal-remote> v1.4.0  # only after explicit authorization
```
