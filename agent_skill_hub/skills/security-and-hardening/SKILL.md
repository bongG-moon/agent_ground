...
- **Run dependency audits only against the approved internal registry, mirror, or offline vulnerability database** using the committed lockfile before every release
...
```
The approved internal/offline dependency audit reports a vulnerability
...
- **Verify signatures and provenance only through the approved internal registry or offline metadata** and treat absence as a signal to investigate, not automatic proof of compromise.
