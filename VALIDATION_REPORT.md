...
> 범위: 기능 단위 Component 20개와 Flow 내부 Node 29개, Hallmark식 디자인 정책·Emil식 모션 검사를 포함한 PPT 참고 이미지 기반 HTML 프레젠테이션 Flow, 입출력·코드 포털, 교육 레이아웃, 이동식 Skill 4종, Flow export 무결성, JSON, Python, HTML, 링크, Registry와 기존 Langflow 계약 회귀
...
| 내부 Node 이동 무결성 | 기존 이동 원본 무결성을 유지하고 디자인 정책 Node 1개를 추가해 총 29개 내부 Node 계약 검증 |
...
| Python 문법 검사 | 95개 통과 |
| Standalone 단일 파일·상대 import 검사 | 기능 Component 20개 + Flow 내부 Node 29개 통과 |
| Component 카탈로그 범위 | General 8개 + Domain 12개 = 기능 단위 20개, Flow 내부 Node 29개는 제외 |
| Component·내부 Node 상세/코드 | Component 20세트와 Flow 내부 Node 29세트에 실제 AST 입력·출력 계약 및 다크 코드 화면, raw `.py` 링크 0개 |
...
| Component와 내부 Node 참조 | `component_refs.json`의 기능 Component 16건과 `internal_nodes.json`의 내부 Node 29건 소유·경로·class·version 통과 |
| Flow refs와 embedded class | runtime-ready Flow는 main/subflow JSON과 자동 대조, 격리 Flow는 명시적 integrity issue 요구 |
| HTML 파싱·구조 | 115개 페이지 통과 |
| 로컬 링크와 자산 경로 | 2,598개 통과 |
| Registry | 기능 Component 20 + 최상위 Flow 5 = 25개 자산 일치, 내부 Node 29개 제외 |
| JavaScript 문법 | 통과 |
...
| 전체 독립 회귀 테스트 | 전체 `pytest` 79개 통과, 실제 Langflow 1.8.2/LFX 0.3.4 환경과 6개 Flow Bundle 순서 포함 |
...
전용 Node·Renderer 테스트 24개와 전체 회귀 테스트 79개, Flow 생성기 `--check`, Python compile, 생성 포털, Project 검증과 workspace audit를 통과했습니다. 다만 실제 Vision 모델/API Key를 연결한 Builder E2E와 생성 발표물의 브라우저 시각 회귀는 아직 수행하지 않아 `user_testing`으로 남깁니다.
