...
| 신규 문서 텍스트 추출(DRM 자동) | 2 nodes / 1 edge, 로컬 우선·항상 DRM·DRM 미사용 모드, 문서·텍스트·일반 이미지 다중 업로드, host allowlist, LLM 없는 Message 출력 |
...
| 전체 독립 회귀 테스트 | 전체 `pytest` 102개 통과, 실제 Langflow 1.8.2/LFX 0.3.4 환경과 7개 Flow Bundle 순서 포함 |

## DRM 문서 텍스트 추출 Flow

`drm_document_text_extraction_flow`는 `drm_document_text_extractor` Standalone Component와 Chat Output만 연결한 최소 Flow입니다. `자동(로컬 우선)`은 PDF·DOCX·PPTX·XLSX·TXT·CSV를 Langflow 환경에서 먼저 읽고 성공한 일반 파일은 외부로 전송하지 않습니다. 로컬 해석 실패 파일과 HWP·구형 Office·이미지 등 로컬 미지원 형식만 DRM API로 보내며, `항상 DRM API`는 모든 파일을 API로 보내고 `DRM 미사용`은 네트워크 호출 없이 로컬 지원 형식만 처리합니다. DRM 요청이 필요할 때는 multipart `file`, `Authorization: Bearer ...`, `empNo` query parameter를 제공 코드와 같은 계약으로 사용합니다. 이미지의 문자 내용은 DRM API가 OCR 평문을 반환할 때 읽을 수 있습니다.
...
가짜 DRM transport를 사용한 전용 테스트에서 UTF-8·CP949 평문, 다중 파일 순서, multipart 요청, HTTP opt-in, allowlist, 확장자·크기 제한과 비밀값 비노출을 확인했습니다. 실제 DOCX·PPTX·XLSX·TXT 샘플은 자동 모드에서 로컬 추출되고 transport 호출 수가 0임을 확인했으며, EWS 일반 첨부는 원본 파일 경로를 유지합니다. Langflow `1.8.2` / LFX `0.3.4`의 실제 template 생성기로 14개 입력과 `extracted_text: Message`·`processed_file: Data` 출력, 2-node / 1-edge 직접 업로드 Flow JSON, 원본 코드 embed와 7개 Project Bundle을 재생성·검증했습니다. 직접 업로드 Flow에서는 EWS용 `file_record` 입력을 숨기고 Message 출력만 사용하고, EWS 메일 Flow에서는 일반 첨부는 원본 경로로 통과시키며 DRM API 처리 파일만 UTF-8 TXT 작업 파일로 저장합니다. 실제 사내 DRM endpoint와 보호 업무 문서는 호출하지 않았으므로 상태는 `user_testing`입니다.
...
- [x] 문서 텍스트 추출(DRM 자동) Component의 실제 1.8.2 template, 처리 모드, 다중 FileInput, Secret 입력, Message 출력과 Flow embed 확인
- [x] 가짜 DRM API로 multipart file, Bearer header, empNo, UTF-8·CP949, allowlist, HTTP opt-in과 크기 제한 확인
- [x] 실제 DOCX·PPTX·XLSX·TXT 샘플의 자동 로컬 추출, DRM transport 무호출과 EWS 원본 경로 통과 확인
...
- [ ] 문서 텍스트 추출 Flow를 Builder에 가져오고 실제 일반 파일의 자동 로컬 처리와 보호 샘플의 사내 DRM API fallback 확인
