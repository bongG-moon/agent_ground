...
3. `03 EWS 첨부 처리 모드·텍스트 추출`에서 처리 모드를 선택합니다.
4. 자동 fallback 또는 항상 DRM을 사용하면 DRM API 주소, Bearer 토큰, 사번과 허용 host를 입력합니다.
5. 두 Language Model 노드에 사내 사용 가능 모델을 선택합니다.
6. 제목 키워드와 읽을 메일 수를 지정하고 Chat Input에 정리 기준을 입력합니다.
