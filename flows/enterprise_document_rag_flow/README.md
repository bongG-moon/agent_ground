...
사내 규정·매뉴얼·FAQ를 질문할 때, 사용자가 볼 수 있는 문서만 검색하고 답변 문장과 출처를 함께 반환하는 Langflow `1.8.2` 실행 예시입니다. 모든 Custom Component는 파일 하나만으로 등록되는 Standalone 방식입니다.
...
- `component_refs.json`: 필요한 9개 Standalone Component
- `samples/sample_enterprise_documents.json`: 외부 Data 입력 계약 예시
- `samples/TEST_QUESTIONS_AND_EXPECTED.md`: 허용·거절·보안 테스트
- `tests/test_enterprise_document_rag.py`: component·payload·ACL·인용 회귀 테스트
