...
0.1.0 실행 구현은 `analyze_and_rebuild`만 지원합니다. 참고 이미지를 실제 배경으로 그대로 삽입하는 모드는 이미지 안의 민감정보·문구·로고가 결과에 재노출될 수 있어 별도 검토 전까지 제공하지 않습니다.
...
Request Builder는 입력값을 다음 실행용 형태로 다시 정규화합니다. 사용자용 `label`·`format`, `preferred_visual`과 `ordinal`·`identifier` 의미 타입도 0.1.0 결과에 보존합니다.
...
표지 또는 본문 이미지가 없으면 현재 기본 정책은 차단이 아니라 fallback 경고입니다. 사내 운영 정책에서 참고 이미지 재현을 필수로 정하려면 후속 버전에서 `require_reference_images` 입력과 차단 검사를 추가해야 합니다. 0.1.0에는 이 설정이 없습니다.
