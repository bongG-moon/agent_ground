...
참고 이미지의 문구를 복사하는 Flow가 아닙니다. 이미지는 색상, 여백, 타이포그래피 계층, 콘텐츠 그리드와 반복 모티프를 분석하기 위한 디자인 근거로만 사용합니다. 발표 내용과 수치는 사용자가 입력한 `brief`와 `datasets`만 사실 근거로 삼습니다.
...
- Flow 전용 Standalone 내부 Node: 6개
- 출력: 독립 실행 가능한 HTML 원문, 품질 검사 결과, 선택적 Report API 공유 링크
- 검증 범위: Python 단위·통합 계약, Langflow 1.8.2/LFX 0.3.4 template 평가, 16-node/18-edge JSON 내장 source·schema 검증
...
두 Encoder 결과 + brief + datasets
  ...
  -> 04 presentation_plan_generator
       └─ Text Language Model 직접 호출
  -> 05 presentation_plan_normalizer
  -> 06 HTML Presentation Renderer
  -> 07 presentation_quality_gate
  -> 08 presentation_html_source_output
  -> Chat Output
...
| 04 | `presentation_plan_generator` | 디자인 분석과 데이터를 바탕으로 슬라이드·차트·표 계획 생성 |
| 05 | `presentation_plan_normalizer` | 모델 출력을 허용된 schema와 토큰으로 제한하고 기본값 보완 |
| 07 | `presentation_quality_gate` | 렌더링된 HTML의 데이터 근거, 접근성 표식, 외부 의존성과 오버플로 위험 검사 |
| 08 | `presentation_html_source_output` | 검증된 HTML 원문을 Chat Output에서 확인 가능한 Message로 변환 |
